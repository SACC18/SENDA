...
export default function AcademicManager({ userId, onNotify }) {
    ...
    // Cargar las clases que dicta el tutor
    useEffect(() => {
    ...
    // Al seleccionar una clase, cargar sus temas y el estado de visibilidad
    const handleSelectClass = async (classItem) => {
        setSelectedClass(classItem)
        setLoading(true)

        // Traer todos los temas de esa materia
        const { data: allTopics } = await supabase
            .from('topics')
            .select('*')
            .eq('subject_id', classItem.subjects.id || classItem.subject_id)
            .order('unit_number', { ascending: true })
            .order('id', { ascending: true })

        // Traer los estados activos para esta clase específica
        ...
        const mergedTopics = allTopics.map(topic => {
            ...
            return { ...topic, is_active: status ? status.is_active : false }
        })
